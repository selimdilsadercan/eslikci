...
const WyrmspanHorizontalScorepad: React.FC<WyrmspanHorizontalScorepadProps> = ({
  ...
  const [playerScores, setPlayerScores] = useState<{[playerId: string]: WyrmspanScores}>({});

  const gamePlayers = players || [];

  const updatePlayerScore = (playerId: string, category: keyof WyrmspanScores, value: number) => {
  ...
  const ResourceIcon = ({ type }: { type: 'gem' | 'leaf' | 'crystal' }) => {
    ...
    const shapes = {
      ...
      crystal: 'M12 2L15.09 8.26L22 9L17 14L18.18 21L12 17.77L5.82 21L7 14L2 9L8.91 8.26L12 2Z'
    };
  ...
  const scoringCategories = [
    { key: 'dragonGuildMarkers' as keyof WyrmspanScores, label: 'Dragon Guild', icon: <ScoreIcon /> },
    { key: 'printedDragons' as keyof WyrmspanScores, label: 'Printed Dragons', icon: <ScoreIcon /> },
    { key: 'endGameAbilities' as keyof WyrmspanScores, label: 'End-game Abilities', icon: <FlagIcon /> },
    { key: 'eggs' as keyof WyrmspanScores, label: 'Eggs*', icon: <ScoreIcon value={1} /> },
    { key: 'cachedResources' as keyof WyrmspanScores, label: 'Cached Resources*', icon: <ScoreIcon value={1} /> },
    { key: 'tuckedCards' as keyof WyrmspanScores, label: 'Tucked Cards*', icon: <ScoreIcon value={1} /> },
    { key: 'publicObjectives' as keyof WyrmspanScores, label: 'Public Objectives', icon: <ScoreIcon />, subtext: '(ties are friendly)' },
    { key: 'coins' as keyof WyrmspanScores, label: 'Coins', icon: <ScoreIcon value={1} /> },
    { key: 'items' as keyof WyrmspanScores, label: 'Items (per 4)', icon: <ScoreIcon value={1} />, showResourceIcons: true, subtext: '(round down)' }
  ];
          ...
          {scoringCategories.map((category) => (
            <div key={category.key} className="flex min-w-max">
              ...
              <div className="w-48 py-3 px-4 flex items-center border-b border-gray-200 bg-white/90 backdrop-blur-sm">
                ...
                {category.subtext && (
                  <div className="text-xs text-gray-500 italic ml-2">{category.subtext}</div>
                )}
